...
//		}			
	...
	private: bool lies_in_range( const Point& p, const Point& pMin, const Point& pMax ){
      ...
      *
      * @return value of the density estimate.
      */
     public: double kde( const Point& Xq, int nodeIdx = 0 ) {
                cout<<"Current node index: "<<nodeIdx<<endl;

                Node* node = nodesPtrs[ nodeIdx ];
                //cout<<"Current point index: "<< node->pIdx << endl;
                cout<<"Is leaf: ";//<< node -> isLeaf();
                //for (int i = 0; i < ndim; i++)
                  //      cout<<points[ node->pIdx ][i]<<" ";
                ...
                double value_current, value_left, value_right, total;;

                value_current = kernel( Xq, points[ node->pIdx ] );
                value_left    = kde( Xq, node->LIdx );
                value_right   = kde( Xq, node->RIdx );
    ...
    private: double kernel( const vector<double>&a, const vector<double> &b  ) {
                 for (int i = 0; i < ndim; i++)
                         printf("p"); 
                return exp( -distance_squared(a, b) );
             }            
};
