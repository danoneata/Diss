function test_cs_back_mixing(dataset, mix_prop)
...
% Dan Oneata, July 2011
  ...
  root = '';
  filename = [dataset '-' num2str(mix_prop) '.txt'];
  T = 1;
  % Load data:
  ...
  for idd = 1:length(dd);
    ...
    for tt = 1:T,
      ...
      nca_score(tt) = nca_cls_cs_back(X_train, c_train, X_test, c_test, AA, moments, Nc);
    ...
    M = [d mean_nca_score std_nca_score mean_nn_score std_nn_score];
    % Save results:
