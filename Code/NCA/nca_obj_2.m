function [f, df] = nca_obj_2(A, X, c)
%NCA_OBJ Neighbourhood Component Analysis second objective function. 
%Returns function value and the first order derivatives.
%
%     [f, df] = nca_obj_2(A, X, c)
%
% Inputs:
...
%     end
    ...
    if nargout > 1,
      ...
      df = df + bsxfun(@times,dist_all,x_ik) * x_ik' / sum_dist_all ...
          - bsxfun(@times,dist_neigh,x_ij) * x_ij' / (p_i * sum_dist_all);
    end
  end
