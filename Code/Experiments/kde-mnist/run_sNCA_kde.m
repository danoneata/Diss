function [mapping, to_plot] = run_sNCA_kde(obj, X, c, d, opts)
%RUN_SNCA 
%
...
% Dan Oneata, July 2011
  ...
  if ~exist('opts','var') || numel(opts) ~= 7,
    opts = [0 50 0.05 0.5 50 3000 1e-7];
  end
  ...
  [lambda, ct] = tune_learning_rate(A, X, c, X_cv, c_cv, obj, m, max_error)
%   score_total = zeros(1,max_iter);
  ...
  while epsilon > 0 && it <= max_iter && ~converged,
    ...
    idxs = randperm(N);
    for ii = 1:m:N,
      [f, df] = feval(obj, A, X, c, idxs(ii:min(N, ii+m-1)), max_error);
...
function [lambda,ct] = tune_learning_rate(Ainit, X, c, X_cv, cv, obj, m, max_error)
  ...
  idxs = randperm(N);
  [dummy, df] = feval(obj, Ainit, X, c, idxs(1:m), max_error);
