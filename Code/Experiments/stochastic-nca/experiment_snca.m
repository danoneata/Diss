...
m = 50;           % Number of points to look at per iteration.
...
NCA_OBJ = 'nca_obj_o1';                               % Objective function.
NR_REPEATS = 5;                                       % Number of repeats.

if ~isunix,
  root_path = 'D:\Diss\Results\stochastic-nca\';
  ff = 'results-windows.txt';
else
  root_path = '~/Documents/Diss/Results/stochastic-nca/';
  ff = 'results-unix.txt';
end
...
for i = 1:length(DATASET),
  for j = 1:NR_REPEATS,
    ...
    tic;
    [mapping, to_plot] = run_sNCA(NCA_OBJ, X, c, d, [init m p lambda t0 max_iter window_len]);
    total_time = toc;
...
%     image_nr = ceil(rand*1000);
    ...
    % fprintf(fid, '---\n');
    fprintf(fid, '%2.2f %2.2f %2.2f %10.3f 2\n',...
